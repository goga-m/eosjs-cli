 ...
 * @returns {Promise<String>} Selected account to get info
 */
const balances = (type = 'list') => {
  // Depending on the type,
  // this prompt will be a list of the existing user
  // or a custom input prompt
  const balanceUser = [{
    type,
    name: 'name',
    message: type === 'list'
    ? 'Select account to retrieve balances for'
    : 'Insert an account name',
    choices: _.concat('Custom', _.map('name', db.getAccounts()))
  }]
  ...
  return inquirer.prompt(balanceUser)
  .then(({name}) => {
    const exists = _.some({name}, db.getAccounts())
    if (!exists && name === 'Custom') {
      return balances('input')
    }
    return inquirer.prompt(currency)
    .then(({currency}) => ({ name, currency }))
  })
}
